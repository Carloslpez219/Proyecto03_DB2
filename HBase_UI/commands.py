            ...
            if checkFile(table_name):
                ...
                if checkRowId(data_table, row_id):
                    if column in data_table["Rows"][row_id]:
                        # Encontrar y eliminar el elemento con el timestamp especificado
                        column_data_list = data_table["Rows"][row_id][column]
                        data_table["Rows"][row_id][column] = [entry for entry in column_data_list if entry["timestamp"] != timestamp]
                        # Eliminar la columna si no quedan versiones
                        if not data_table["Rows"][row_id][column]:
                            del data_table["Rows"][row_id][column]
                        # Eliminar la fila si no quedan columnas
                        if not data_table["Rows"][row_id]:
                            del data_table["Rows"][row_id]
                        with open(f"./HFiles/{table_name}.json", "w") as file:
                            json.dump(data_table, file, indent=4)
                        return f"Version with timestamp '{timestamp}' deleted from column '{column}' in row '{row_id}' from table '{table_name}'."
    return "Invalid command"
